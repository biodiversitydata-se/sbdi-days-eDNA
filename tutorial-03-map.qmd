...
Samples are ordered consistently across tables, which makes it straightforward to link sequence data with contextual data. Thus, `rownames(merged_df$events)`,  `rownames(merged_df$emof)` and `colnames(merged_df$emof)` are all the same. Likewise ASVs are ordered the same way in `merged_df$counts` and `merged_df$asvs`.
...
Additional contextual parameters are available in `merged_df$emof`. Unlike other metadata tables with fixed core columns, EMoF variables are dataset-specific and may need to be explicitly harmonised before analysis:
